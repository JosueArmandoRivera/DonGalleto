    ...
    public function armarTabla()
    {
      //  dd('Entra al método armarTabla');
       ...
       //dd('Entra al try');
        ...
        $tabla = '<div class="table-responsive"><table id="tablaUsuarios" class="table table-hoverdisplay table-striped table-hover no-wrap" width="100%">
                                <thead class="bg-dark">
                                    <tr>
        ...
        try {    //Abrimos un catch
            ...
            $query = DB::select('EXEC SP_Usuarios_Seleccionar ?', [Auth::user()->Id_Usuario]);   //Ejecutamos el SP para seleccionar todos los registros
            ...
            if (empty($query)) {    //Si la variable query esta vacía
                ...
                if (property_exists($query[0], 'respuesta')) {  //Si la variable query contiene el atributo respuesta significa que tiene un error
                    ...
                    foreach ($query as $usuarios) {   //Abrimos un foreach y vamos armando todas las columnas de la tabla
                        $tabla .= '<tr>
                                    <td class="d-none">' . $usuarios->Id_Usuario . '</td>
                                    <td title="' . $usuarios->Nombres . '">' . $usuarios->Nombres . '</td>
                                    <td title="' . $usuarios->Telefono_Personal . '">' . $usuarios->Telefono_Personal . '</td>
                                    <td title="' . $usuarios->Whatsapp . '">' . $usuarios->Nombre_Area . '</td>
                        ...
                        if ($permisoConsultar || $permisoModificar || $permisoEliminar) {
                            ...
                            if ($usuarios->Disponible == '1') {
                                ...
                                if ($permisoConsultar) {    //Si el permiso de consultar es true, agregamos el boton para los detalles
                                    $tabla .= ' <a href="#" idUsuario="' . $usuarios->Id_Usuario . '" id="verArea" title="Detalles de la unidad administrativa" class="btn btn-sm  btn-default text-dark  m-1 p-1 shadow"> <i class="fa-solid fa-eye fa-lg"></i><a/>';
                                }
                                if ($permisoModificar) {    //Si el permiso de modificar el true, agregamos el boton para la modificacion del area
                                    $tabla .= '<a href="#" idUsuario="' . $usuarios->Id_Usuario . '"  id="editarArea" title="Modificar unidad administrativa" class="btn btn-sm  btn-default text-dark m-1 p-1 shadow"> <i class="fa-solid fa-pen-to-square fa-lg"></i></a>';
                                }
                                if ($permisoEliminar) { //Si el permiso eliminar es true, agregamos el botón en el apartado de acciones
                                    $tabla .= '<a href="#" idUsuario="' . $usuarios->Id_Usuario . '" id="eliminarArea" title="Eliminar unidad administrativa" class="btn btn-sm btn-default text-dark m-1 p-1 shadow"><i class="fa-solid fa-trash-can fa-lg"></i></a>';
                                }
                            } else {    //En caso de que el estatus sea diferente de 1
                                //Agregamos una columna en tabla con la palabra deshabilitada
                                $tabla .= '<td title="Disponible"><i class="fa-solid fa-circle" style="color: #ff0000;"></i> Disponible</td>
                                                            <td class="text-center">';
                                if ($permisoConsultar) {    //Si el permiso de consultar es true, agregamos el boton para los detalles
                                    $tabla .= ' <a href="#" idUsuario="' . $usuarios->Id_Usuario . '" id="verArea" title="Detalles de la categoría" class="btn btn-sm  btn-default text-dark  m-1 p-1 shadow"> <i class="fa-solid fa-eye fa-lg"></i><a/>
                                                                ';
                                }
                            }
